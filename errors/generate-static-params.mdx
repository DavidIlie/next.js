...
- returns params that do not produce any routes. For example, `{ slug: 'first' }` does not provide the required `id` value for a route using an `[id]` segment.

Export `generateStaticParams` from every dynamic route and return at least one params object that provides every required dynamic parameter. If the paths cannot be known at build time, remove `output: 'export'` from your Next.js configuration.
