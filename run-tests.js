...
const { buildTestReport, datadogTestRunUrl } = require('./scripts/test-report')
...
async function maybeLogSummary() {
  if (process.env.CI && errorsPerTests.size > 0) {
    ...
    // Mirrors the bundler detection in `getJobTags`/`formatFailureLine` in
    // `scripts/pr-ci-comment.mjs`, which derive it from the CI job name.
    const isRspack = !!process.env.NEXT_RSPACK
    const testType = process.env.IS_TURBOPACK_TEST ? 'turbopack' : 'nextjs'

    const suites = [...errorsPerTests.keys()].sort().map((test) => {
      const { output, failedCases } = errorsPerTests.get(test)
      return {
        title: `\`${getSummaryTestCommand(test)}\``,
        failureLines: failedCases.map((caseName) => {
          // No Datadog links for rspack jobs, mirroring `formatFailureLine`
          // in `scripts/pr-ci-comment.mjs`.
          if (isRspack || !owner || !repo || !sha) {
            return caseName
          }
          const linkUrl = datadogTestRunUrl({
            ...
            repo,
            sha,
            testName: caseName.replace(/ > /g, ' '),
            testType,
          })
          return `${caseName} ([DD](${linkUrl}))`
        }),
