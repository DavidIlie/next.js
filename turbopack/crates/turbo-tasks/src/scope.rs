...
struct UnboundedInner<T: Send + 'static> {
    ...
    /// The per-item closure (with turbo-tasks context re-established), shared by every drainer.
    /// Its `'env` borrows are erased to `'static` here; the `Joiner` guarantees every drainer
    /// finishes before `'env` ends. See `scope_unbounded` for the erasure's safety argument.
    run: Box<dyn Fn(&Spawner<'_, T>, T) + Send + Sync>,
}
...
impl<T: Send + 'static> UnboundedInner<T> {
    ...
    fn wait(&self) {
        ...
        // TODO: a single long-running tail item can leave other drainers parked here while it runs.
        // Consider a timeout/steal strategy if that becomes a problem in practice.
        const TIMEOUT: Duration = Duration::from_millis(1);
        let beginning_park = Instant::now();
...
fn enqueue<T: Send + 'static>(inner: &UnboundedInner<T>, item: T) {
    ...
    inner.work_queue.lock().push_back(item);
...
where
    T: Send + 'static,
    F: Fn(&Spawner<'_, T>, T) + Send + Sync + 'env,
{
    let handle = Handle::current();
    ...
    // The items are `'static`, so the ONLY thing borrowing `'env` is `run`. Erase its borrows to
    // `'static` once (not once per item) so `UnboundedInner<T>` — and the `Arc` clones helper tasks
    // hold — are `'static`. SAFETY: the `Joiner` below drains and joins every helper before this
    // function returns (before `'env` ends), so no drainer can invoke `run` past `'env`. This is
    // `Scope::spawn`'s `'env`->`'static` transmute, applied to the single shared `run`.
    let run: Box<dyn Fn(&Spawner<'_, T>, T) + Send + Sync + 'env> = Box::new(wrapped_run);
    let run: Box<dyn Fn(&Spawner<'_, T>, T) + Send + Sync + 'static> = unsafe {
        std::mem::transmute::<
            Box<dyn Fn(&Spawner<'_, T>, T) + Send + Sync + 'env>,
            Box<dyn Fn(&Spawner<'_, T>, T) + Send + Sync + 'static>,
        >(run)
    };
